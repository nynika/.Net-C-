...
namespace BALayer
{
    public class MediBusinessLayer : IMediBusiness
    {
        ...
        public List<res_opd_Process> Get_opd_Process()
        {
            ...
            DataSet ds = dataLayer.Get_opd_Process();
            if (ds != null)
            {
                if (ds.Tables.Count > 0 && ds.Tables[0] != null && ds.Tables[0].Rows != null)
                {
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        ...
                        res_opd_Process result = new res_opd_Process();

                        result.OPD_Prcess_nbr = DBNull.Value != dr["OPD_Prcess_nbr"] ? Convert.ToInt32(dr["OPD_Prcess_nbr"]) : 0;
                        result.PatientName = DBNull.Value != dr["PatientName"] ? Convert.ToString(dr["PatientName"]) : "";
                        result.UHID = DBNull.Value != dr["UHID"] ? Convert.ToString(dr["UHID"]) : "";
                        result.VisitNo = DBNull.Value != dr["VisitNo"] ? Convert.ToString(dr["VisitNo"]) : "";
                        result.VisitDate = DBNull.Value != dr["VisitDate"] ? Convert.ToString(dr["VisitDate"]) : "";
                        result.Rad_Order = DBNull.Value != dr["Rad_Order"] ? Convert.ToString(dr["Rad_Order"]) : "";
                        result.Study_Start = DBNull.Value != dr["Study_Start"] ? Convert.ToString(dr["Study_Start"]) : "";
                        result.Study_Authorized = DBNull.Value != dr["Study_Authorized"] ? Convert.ToString(dr["Study_Authorized"]) : "";
                        result.Lab_Order = DBNull.Value != dr["Lab_Order"] ? Convert.ToString(dr["Lab_Order"]) : "";
                        result.Sample_Collected = DBNull.Value != dr["Sample_Collected"] ? Convert.ToString(dr["Sample_Collected"]) : "";
                        result.Lab_Authorized = DBNull.Value != dr["Lab_Authorized"] ? Convert.ToString(dr["Lab_Authorized"]) : "";
                        result.Admission_Status = DBNull.Value != dr["Admission_Status"] ? Convert.ToString(dr["Admission_Status"]) : "";
                        result.Medicines_Prescribed = DBNull.Value != dr["Medicines_Prescribed"] ? Convert.ToString(dr["Medicines_Prescribed"]) : "";
                        result.Billing_status = DBNull.Value != dr["Billing_status"] ? Convert.ToString(dr["Billing_status"]) : "";

                        result.Vitals_Completed_time = DBNull.Value != dr["Vitals_Completed_time"] ? Convert.ToString(dr["Vitals_Completed_time"]) : "";
                        result.Doctor_Checkin = DBNull.Value != dr["Doctor_Checkin"] ? Convert.ToString(dr["Doctor_Checkin"]) : "";
                        result.Followup_Appointment = DBNull.Value != dr["Followup_Appointment"] ? Convert.ToString(dr["Followup_Appointment"]) : "";
                        result.Followup_Billed = DBNull.Value != dr["Followup_Billed"] ? Convert.ToString(dr["Followup_Billed"]) : "";
                        result.Admission_advised = DBNull.Value != dr["Admission_advised"] ? Convert.ToString(dr["Admission_advised"]) : "";

                        result.Rad_Investigations_Billed = DBNull.Value != dr["Rad_Investigations_Billed"] ? Convert.ToString(dr["Rad_Investigations_Billed"]) : "";
                        result.Lab_Investigations_Billed = DBNull.Value != dr["Lab_Investigations_Billed"] ? Convert.ToString(dr["Lab_Investigations_Billed"]) : "";
                        result.Proc_Billed = DBNull.Value != dr["Proc_Billed"] ? Convert.ToString(dr["Proc_Billed"]) : "";
                        result.Procedure_Advised = DBNull.Value != dr["Procedure_Advised"] ? Convert.ToString(dr["Procedure_Advised"]) : "";
